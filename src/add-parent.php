...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $status = 1; // Handle optional field

    // Check if the email already exists in the database
    $emailCheckQuery = "SELECT * FROM `parents` WHERE `email` = ?";
    ...
    if ($emailResult->num_rows > 0) {
        ...
        if (empty($errormessage)) {
            ...
            $sql = "INSERT INTO `parents`(`fname`, `mname`, `lname`, `genderid`, `email`, `tel`, `address`, `password`, `status`, `photo`) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            // Bind the parameters
            $stmt->bind_param("sssissssis", $fname, $mname, $lname, $genderid, $email, $phone, $address, $password, $status, $photo);

            // Execute the statement
            if ($stmt->execute()) {
                $_SESSION['alert'] = [
                    'type' => 'success',
                    'message' => 'Parent\'s details have been successfully inserted!'
                ];
                header("Location: " . $_SERVER['PHP_SELF']); // Redirect to avoid resubmission on page reload
                exit;
            } else {
                $errormessage = "Failed to insert the parent's details. Please try again.";
            }
        }
    }
}
...
<main class="py-5">
    <div class="container" style="display: grid; grid-template-columns: 300px auto; column-gap: 10px; background-color: var(--darkgrey);">
        ...
        <div class="p-4">
            <h2 class="page-title">New Parent</h2>
            <br>
            ...
            <form class="registration-form" method="POST" enctype="multipart/form-data">
                ...
                <span class="d-flex column-gap-2">
                    ...
                    <a href="all-parents.php" class="btn btn-secondary w-50">Cancel</a>
                </span>
